...
题目链接：[LeetCode](https://leetcode.com/problems/distance-between-bus-stops/description/)  /  [LeetCode中国](https://leetcode-cn.com/problems/distance-between-bus-stops/description/)
...
class Solution:
    def distanceBetweenBusStops(self, distance: List[int], start: int, destination: int) -> int:
        if start == destination:
            return 0
        b, a = (start, destination) if start > destination else (destination, start)
